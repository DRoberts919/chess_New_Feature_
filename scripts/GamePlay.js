 ...
 */
  ...
  that.init = function (userSettings) {
    ...
    if (container == null) {
      console.log(
        "container element not found with id: " + _settings["containerID"]
      );
      return false;
    }
    ...
    CHESSAPP.ui.onInitialChoice(function (pref) {
      ...
      console.log(pref.chess960);
      if (pref.hasOwnProperty("online")) {
        _settings.online = pref.online;
      }
      if (pref.hasOwnProperty("chess960")) {
        _settings.chess960 = pref.chess960;
      }
      ...
      } else if (_settings.chess960) {
        CHESSAPP.ui.statusUpdate({ type: "fb", msg: "Playing Chess960" });
        that.setUpBoard(true);
      } else if (!_settings.online) {
        CHESSAPP.ui.statusUpdate({
          type: "fb",
          msg: "Playing locally",
        });
  ...
  const randomPos = function (min, max) {
    let ranNum = Math.floor(Math.random() * (max - min + 1)) + min;
    return ranNum;
  };

  const checkPos = function (pieceArray, piece) {
    console.log(pieceArray);
    for (let i = 0; i < pieceArray.length; i++) {
      console.log(pieceArray[i]);
      if (pieceArray[i] == piece) {
        console.log(i);
        return i;
      } else if (pieceArray[i] == piece) {
        return i;
      }
    }
  };

  const checkOpenSpots = function (boardArray) {
    let openspaces = [];
    boardArray;
    for (let i = 0; i < boardArray.length; i++) {
      if (boardArray[i] == "spot") {
        openspaces.push({
          space: i,
  ...
  that.setUpBoard = function (chess960Check) {
    let boardPositions = [
      "spot",
      "spot",
      "spot",
    ...
    console.log(chess960Check);
    //create pieces
    if (chess960Check) {
      // this setups the chess 960 pieces

      randomPieces.forEach((element) => {
        if (element.pieceType == "king" && element.color == "W") {
          let pos = randomPos(1, 6);

          element.x = pos;

          boardPositions[pos] = "king";
        } else if (element.pieceType == "rook" && element.color == "W") {
          // get back the position of
          let kingpos = checkPos(boardPositions, "king");

          // place first rook

          if (element.side == "left") {
            let posLeft = randomPos(1, kingpos);
            element.x = posLeft;

            // fill boardPos Array with rook pos
            boardPositions[posLeft] = "rook";
          }

          // place right rook
          if (element.side == "right") {
            let posRight = randomPos(kingpos + 1, 7);
            boardPositions[posRight] = "rook";
            element.x = posRight;
          }
        } else if (
          element.pieceType == "bishop" &&
          element.color == "W" &&
          element.num == "One"
        ) {
          let openspaces = checkOpenSpots(boardPositions);
          // console.log(openspaces);
          let pickedPos;

          openspaces.forEach((spaces) => {
            if (spaces.space % 2 != 0) {
              pickedPos = spaces.space;
            }
          });

          element.x = pickedPos;

          boardPositions[pickedPos] = "bishop";
          ...
          boardPositions[pickedPos] = "bishop";
        } else if (element.color == "W" && element.pieceType == "knight") {
          let openSpot = checkOpenSpots(boardPositions);

          let ranpos = randomPos(0, openSpot.length - 1);

          let pickedPlace = openSpot[ranpos];

          element.x = pickedPlace.space;

          boardPositions[pickedPlace.space] = element.pieceType;
        } else if (element.color == "W" && element.pieceType == "queen") {
          let spot = checkOpenSpots(boardPositions);

          element.x = spot[0].space;
          boardPositions[spot[0].space] = "queen";
        } else if (element.color == "B") {
          console.log(boardPositions);
          console.log(element.pieceType);

          for (let i = 0; i < boardPositions.length; i++) {
            if (boardPositions[i] == element.pieceType) {
              element.x = i;
            }
          }
        }
      ...
      that.pieces = randomPieces;
    } else {
      // this setups the regular chessgame
      that.pieces = [
        {
          x: 0,
          y: 7,
  ...
  that.movePieceTo = function (stg) {
    ...
    if (stg.special != null) {
      ...
      if (stg.special.type == "en") {
        ...
        //move that rook
        var rook = CHESSAPP.Analyzer.pieceExists({
          pieces: that.pieces,
          x: stg.special.rookx,
          y: stg.special.rooky,
        });
    ...
    (that.promote = function (stg) {
      ...
      if (pieceGettingPromoted) {
        ...
        if (local || !_settings.online) {
          ...
          that.statusUpdate({
            msg: "Your opponent has been promoted",
            type: "e",
          });
        }
  ...
  that.onlineMove = function (data) {
    ...
    var pieceMoved = CHESSAPP.Analyzer.pieceExists({
      pieces: that.pieces,
      x: data.pieceX,
      y: data.pieceY,
    });
...
var statusScroller = function (stg) {
  ...
  this.move = function (up) {
    if (stg.scroll) {
      return;
    } //this is only for non scrolling
    if (totalLines <= maxLines) {
      return;
    }
    if (up) {
