...
CHESSAPP.ui = (function () {
  ...
  var createColor = function () {
    ...
    span.appendChild(
      document.createTextNode(
        "Matches up with anyone, either black or white player"
      )
    );
    a3.appendChild(span);
  ...
  createInitial = function () {
    ...
    // my code for new chess game
    a3.setAttribute("data-mode", "chess960");
    a3.appendChild(document.createTextNode("Chess960"));
  ...
  var readyToPlay = function () {
    ...
    initSub.apply(window, [
      { color: preferredColor, online: online, chess960: chess960 },
    ]);
  };
  ...
  that.addChatMessage = function (stg) {
    var prefix =
        stg.color == "W" ? "White - " : stg.color == "B" ? "Black - " : "",
  ...
  that.drawPieces = function (pieces, cells) {
    ...
    for (; i < max; i++) {
      ...
      var img = new Image();
      img.src =
        CHESSAPP.globalSettings.imageDir + p.color + "_" + p.pieceType + ".png";
      p.reference = img;
  ...
  that.modeClicked = function (e) {
    ...
    if (src.nodeName.toLowerCase() == "a") {
      ...
      // } else if (val == "online") {
      //   online = true;
      // } else if ((val = "chess960")) {
      //   chess960 = true;
      // }

      // this is my code adjustment to take into account for new 960 gamemode
      console.log(`modeClicked val ==  ${val}`);
      if (val == "chess960") {
        chess960 = true;
      } else if (val == "offline") {
        online = false;
  ...
  that.boardClicked = function (e) {
    ...
    if (cellReference) {
      ...
      if (x && y) {
        ...
        var piece = CHESSAPP.Analyzer.pieceExists({
          pieces: CHESSAPP.GamePlay.pieces,
          x: x,
          y: y,
        });
        if (piece) {
